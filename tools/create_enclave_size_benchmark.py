    ...
    try:
        ...
        for requested_size in args.size:
            ...
            result = {
                ...
                'create_enclave_cycles_mean': round(mean(sample['create_enclave_cycles'] for sample in samples), 3),
                'create_enclave_cycles_stddev': round(stdev(sample['create_enclave_cycles'] for sample in samples), 3) if len(samples) > 1 else 0.0,
                'aes_decrypt_cycles_mean': round(mean(sample['aes_decrypt_cycles'] for sample in samples), 3),
                ...
                'destroy_atomic_cycles_stddev': round(stdev(sample['destroy_atomic_cycles'] for sample in samples), 3) if len(samples) > 1 else 0.0,
            }
