...
def send_create(device: UartDevice) -> float:
    return send_create_with_size(device, 0)


def send_create_with_size(device: UartDevice, decrypt_size: int) -> float:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Benchmark only create_enc')
    ...
    parser.add_argument('--size', type=int, nargs='+', default=[39552], help='Decrypt sizes in bytes to benchmark; use 39552 for the full late-weight blob')
    parser.add_argument('--runs', type=int, default=10, help='Number of create/destroy attempts (default: 10)')
...
def main() -> int:
    ...
    for requested_size in args.size:
        if requested_size <= 0:
            raise ValueError('--size values must be > 0; use 39552 for the full late-weight blob')

    output_path = Path(args.output) if args.output else Path(__file__).resolve().parents[1] / 'build' / 'create_enclave_benchmark.json'
    ...
    try:
        ...
        for requested_size in args.size:
            ...
            for attempt in range(args.runs):
                host_create_ms = send_create_with_size(device, requested_size)
            ...
            result = {
                ...
                'runs': int(args.runs),
                'samples': samples,
                'create_enc_cycles_mean': round(mean(sample['create_enc_cycles'] for sample in samples), 3),
                'create_enc_cycles_stddev': round(stdev(sample['create_enc_cycles'] for sample in samples), 3) if len(samples) > 1 else 0.0,
                ...
                'destroy_enc_cycles_mean': round(mean(sample['destroy_enc_cycles'] for sample in samples), 3),
                'destroy_enc_cycles_stddev': round(stdev(sample['destroy_enc_cycles'] for sample in samples), 3) if len(samples) > 1 else 0.0,
            ...
            print(f"size={requested_size}B create_enc: {result['create_enc_cycles_mean']} ± {result['create_enc_cycles_stddev']} cycles")
        ...
        print(f'[OK] create_enc benchmark written: {output_path}')
        return 0
    finally:
        device.disconnect()
